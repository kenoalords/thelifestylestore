 ...
 * @requires ../../../node_modules/slick-carousel/slick/slick.min.js
 * @requires ../../../node_modules/toastr/toastr.js
 */
 ...
 (function(window, document, $, undefined){
    ...
    $('.add_to_cart').on('submit', function(e){
        e.preventDefault()
        var $this = $(this)
        $this.find('button').addClass('is-loading').attr('disabled', 'disabled')
        var url = $this.attr('action')
        var data = {
            'quantity': $this.find('.quantity').val(),
            'pk': $this.find('.product_id').val()
        }
        $.post(url, data, function(res){
            ...
            $this.find('button').removeClass('is-loading').removeAttr('disabled', 'disabled')
        })
    })
    ...
    $('.rating-form').on('submit', function(e){
        ...
        $.post(url, data, function(res){
            if ( res.status === true ){
                ...
                toastr.success('You like this product');
                button.removeClass('is-loading').removeAttr('disabled');
            } else {
    ...
    $('.cart_widget').on('click', function(e){
        e.preventDefault();
        var $this = $(this),
            qty = $this.find('.quantity'),
            current_quantity = qty.val(),
        ...
        if ( e.target.classList.contains('plus') ){
            if ( current_quantity < max ){
                ...
                alert('You\'ve selected more than the available quantity');
                return;
            }
        }
    ...
    $('#newsletter-email').on('focus', function(e){
        $('.newsletter-extras').fadeIn('fast')
    });

    $('#email-sub-form').on('submit', function(e){
        e.preventDefault();
        var email = $.trim( $('#newsletter-email').val() ),
            fname = $.trim( $('#newsletter-fname').val() ),
            lname = $.trim( $('#newsletter-lname').val() ),
            url = 'https://thelifestylestore.us19.list-manage.com/subscribe/post-json?c=?',
            u = $('#newsletter-u').val(),
            id = $('#newsletter-id').val();
        if( fname == '' || lname == '' ){
            toastr.error('Please provide your First and Last name')
            return;
        }
        if ( !checkEmail(email) ){
        ...
        $('#newsletter-submit').addClass('is-loading').attr('disabled', 'disabled');
        var data = {
            'MERGE0': email,
            'MERGE1': fname,
            'MERGE2': lname,
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                toastr.success('You have now subscribed for our newsletter', 'Thank you');
            },
            error: function(){
                toastr.error('There was a problem signing you up. Please try again');
            }
        }).always(function(response){
            $('#newsletter-submit').removeClass('is-loading').removeAttr('disabled');
        })
